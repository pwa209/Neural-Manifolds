...
usage() {
  note "Usage: $0 --canonical-root PATH --work-root PATH --checkpoint-root PATH --repo-root PATH --python PATH --run-id ID [--server-config ABSOLUTE_PATH] [--from-phase NAME|--through-phase NAME|--only-phase NAME] [--fmri-input-manifest ABSOLUTE_PATH] (--check-only|--dry-run|--apply)"
}

canonical_root=""; work_root=""; checkpoint_root=""; repo_root=""
python_bin=""; run_id=""; mode=""; from_phase=""; through_phase=""; only_phase=""
fmri_input_manifest=""; server_config=""
while (($#)); do
  case "$1" in
