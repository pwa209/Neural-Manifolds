...
Source repair (24 September 2026): the downloaded *Dream_YoungAdults* ZIP matches Figshare file `39673915`'s published MD5 (`f4fba4f3b78ce13d54ad424ed2c09aef`). Two source EDF entries, `DG41.edf` and `DM02.edf`, nevertheless have invalid local ZIP headers. Re-downloading that published version would not restore those entries. The repaired preflight now records each as a named, unreadable-source exclusion and continues with readable participants. It does not salvage, recode, or silently count either entry; the archive and failed first run are preserved. Measurement also records any later ZIP read failure as unavailable.

The first repaired run passed preflight (128 candidate people across six cohorts) and then found that some MNE sample indices are NumPy integers which the checkpoint JSON writer did not accept. A second source repair converts audit scalars to JSON-native values without changing EEG samples, feature calculations, labels, or exclusions. The first two run directories and their receipts remain intact; measurement is restarted in a separate run.

In the third run, full-file reading found a further published YoungAdults member (`DG05.edf`) whose CRC fails despite its readable header. The measurement stage records this as unavailable rather than salvaging a prefix. The completed preflight admitted 128 candidate people; signal measurement retained 126. The observed nested analysis completed on all six cohorts, and the two null-refit families are queued/running. No final corrected inference is yet available.
