...
The controller and the scheduled assistant continuation are complementary. The
controller submits implemented tasks to Slurm and reconciles durable receipts.
The hourly assistant continuation audits remaining data mappings, implements and
tests missing revised scientific stages, deploys immutable source releases, and
records sanitized progress. A scheduled assistant check is not itself a completed
scientific analysis.
...
- Node/preemption failures receive at most three attempts, with backoff. Software,
  metadata and unknown-scheduler failures remain visible for the continuation to
  diagnose. Acquisition receives at most three supervised restarts per controller
...
2. Add safe bounded RAR handling and full signal QC. Resolve duration, channel,
...
These integrations remain active development work, not silently skipped successful
stages. The hourly continuation is authorized to complete them without a new user
instruction, within the current study design, open-access policy and resource limits.
It must request user action for renewed MFA or a material expansion of authority.
...
only on meaningful change or required action. Desktop-dependent hourly follow-ups
are separate from the already-running remote jobs and controller.
